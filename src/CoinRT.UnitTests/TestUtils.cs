...
namespace CoinRT.UnitTests
{
    public static class TestUtils
    {
        ...
        {
            return blockStore.GetChainHead().Header.CreateNextBlock(EcKey.Create().ToAddress(networkParams));
        }

        /// <exception cref="BlockStoreException"/>
        public static Block MakeSolvedTestBlock(NetworkParameters networkParams, IBlockStore blockStore)
        {
            var b = blockStore.GetChainHead().Header.CreateNextBlock(EcKey.Create().ToAddress(networkParams));
            b.Solve();
            return b;
        }
        ...
        {
            var b = prev.CreateNextBlock(EcKey.Create().ToAddress(networkParams));
            b.Solve();
            return b;
        }
    }
